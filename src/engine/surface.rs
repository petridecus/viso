...
impl VisoEngine {
    ...
    /// cavities). Thin dispatcher into [`regenerate_surfaces`].
    pub(crate) fn regenerate_entity_surfaces(&self) {
        regenerate_surfaces(
            &self.scene,
            &self.annotations,
            &self.density,
...
/// concatenates all meshes, and sends the result to the density mesh
/// channel (shared with density map rendering).
pub(crate) fn regenerate_surfaces(
    ...
    if jobs.is_empty() && density_jobs.is_empty() && cavity_jobs.is_empty() {
        ...
        let _ = gpu.density_channel.tx.send((Vec::new(), Vec::new()));
        return;
    }

    let tx = gpu.density_channel.tx.clone();
    ...
    let _ = std::thread::spawn(move || {
        ...
        for (map, threshold, color) in &density_jobs {
            let (v, i) =
                crate::renderer::geometry::isosurface::density::generate_density_mesh(
