...
impl VisoEngine {
    ...
    /// surface, runs the appropriate surface generator, concatenates
    /// all meshes, and sends the result to the density mesh channel
    /// (shared with density map rendering).
    pub(super) fn regenerate_entity_surfaces(&self) {
        use crate::options::SurfaceKindOption;
        use crate::renderer::geometry::isosurface::{gaussian_surface, ses};
        ...
        for (entity_idx, se) in all_entities.iter().enumerate() {
            ...
            let base_surface = if let Some(s) = self.entity_surfaces.get(&eid) {
                if !s.visible {
                    continue;
                }
                s.clone()
                ...
                match global_kind {
                    SurfaceKindOption::Gaussian => EntitySurface {
                        kind: SurfaceKind::Gaussian,
                        color: [0.7, 0.7, 0.7, global_opacity],
                        ..Default::default()
                    },
                    SurfaceKindOption::Ses => EntitySurface {
                        kind: SurfaceKind::Ses,
                        color: [0.7, 0.7, 0.7, global_opacity],
                        ..Default::default()
                    },
                    SurfaceKindOption::None => continue,
                }
            };
            ...
            // Use the backbone palette so surface colors match the backbone
            let [r, g, b] = palette.categorical_color(entity_idx);
            let mut surface = base_surface;
            surface.color = [r, g, b, surface.color[3]];

            // SES needs a finer grid than Gaussian to resolve
            // atom-level detail (ChimeraX default is 0.5 Å).
            if surface.kind == SurfaceKind::Ses {
                surface.resolution = 0.5;
            }

            jobs.push((positions, radii, surface));
        }
        ...
        if jobs.is_empty() && density_jobs.is_empty() {
