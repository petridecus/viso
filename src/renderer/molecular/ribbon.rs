...
impl RibbonRenderer {
    pub fn new(
        ...
        let (vertices, indices, offsets) =
            Self::generate_from_ca_only(backbone_chains, &params, None);
    ...
    pub fn new_from_residues(
        ...
        let params = RibbonParams::default();
        let (vertices, indices, offsets) =
            Self::generate_from_residues(backbone_chains, &params, None);
    ...
    pub fn update_from_residues(
        ...
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        chains: &[Vec<RenderBackboneResidue>],
    ) {
    ...
    pub(crate) fn generate_from_ca_only(
        chains: &[Vec<Vec3>],
        params: &RibbonParams,
        ss_override: Option<&[SSType]>,
    ) -> (Vec<RibbonVertex>, Vec<u32>, Vec<(u32, Vec3)>) {
    ...
    pub(crate) fn generate_from_ca_only_colored(
        chains: &[Vec<Vec3>],
        params: &RibbonParams,
        ss_override: Option<&[SSType]>,
...
fn generate_helix(
    residues: &[RenderBackboneResidue],
    ss_types: &[SSType],
    params: &RibbonParams,
    base: u32,
...
fn generate_helix_from_ca(
    ...
    let widths = compute_taper_widths(
        frames.len(),
        params.helix_width,
...
fn sheet_from_normals(
    ...
    let tangents: Vec<Vec3> = spline_points
        .iter()
        .enumerate()
        .map(|(i, _)| {
            if i == 0 {
    ...
    let spline_normals: Vec<Vec3> = (0..spline_points.len())
        .map(|i| {
